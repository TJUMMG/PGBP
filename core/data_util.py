...
def index_to_time(start_index, end_index, num_units, extend_pre, extend_suf,
                  duration,pos_index):
    p_times = np.arange(0, num_units).astype(
        np.float32) * duration / float(num_units)
    pos_time = p_times[pos_index - extend_pre]
    start_time = start_index * duration
    end_time = end_index * duration
    return start_time, end_time,pos_time

def index_to_time1(num_units, extend_pre, extend_suf,
                  duration,pos_index):
    p_times = np.arange(0, num_units).astype(
        np.float32) * duration / float(num_units)
    pos_time = p_times[pos_index - extend_pre]
...
def index_to_time2(start_index, end_index, num_units, extend_pre, extend_suf,
                  duration):
    start_time = start_index * duration
    end_time = end_index * duration
    return start_time, end_time
