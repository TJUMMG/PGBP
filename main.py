...
def parse_args():
    ...
    parser.add_argument("--mode", default="train", help="train, test, test_train", type=str)
    ...
    if args.mode.lower() == "train":
        ...
        for epoch in range(config.TRAIN.MAX_EPOCH):
            ...
            for data, anno in dataloader_train:
                global_step += 1
                step += 1
                batch_start_time = data["batch_start_time"].to(device)
                batch_end_time = data["batch_end_time"].to(device)
                batch_duration = data["batch_duration"].to(device)
                ...
                batch_positive_indices = data["batch_positive_indices"].to(device)
                batch_negative_indices = data["batch_negative_indices"].to(device)
                ...
                with torch.cuda.amp.autocast():
                    ...
                    if torch.cuda.device_count() > 1:
                        contrast_loss = model.module.compute_sim_loss(
                            output[5],
                            batch_positive_indices,
                            batch_negative_indices
                        )

                        kl_loss = model.module.aligment_score(
                            output[3],
                            output[4],
                            batch_txt_mask,
                            batch_vis_mask,
                        ...
                        kl_loss2 = model.module.aligment_score(
                            output[3],
                            output[6],
                            batch_txt_mask,
                            batch_vis_mask,
                    ...
                    # )
                        ...
                        loc_loss, match_loss,iou_loss,bce_loss = model.module.compute_loss(
                            start_logits,
                            end_logits,
                            additional_logits,
                            batch_start_time,
                        ...
                        early_loss = model.module.early_pred_loss(
                            output[4], output[5], batch_internel_label, batch_vis_mask
                        )
                        cont_loss =  model.module.PNcontrast_loss(
                            batch_vis_mask,
                            output[-3],
                            output[-2],
                            batch_internel_label,
                            output[-1],
                            config.LOSS.hy_sigma,
                            config.LOSS.contrast_weight
                                 )
                    else:
                        contrast_loss = model.compute_sim_loss(
                            output[5],
                            batch_positive_indices,
                            batch_negative_indices
                        )
                    ...
                    # )
                        ...
                        loc_loss, match_loss,iou_loss,bce_loss = model.compute_loss(
                            start_logits,
                            end_logits,
                            additional_logits,
                            batch_start_time,
                        ...
                        cont_loss =  model.PNcontrast_loss(
                            batch_vis_mask,
                            output[-3],
                            output[-2],
                            batch_internel_label,
                    ...
                    total_loss = (
                        ...
                        +config.LOSS.contrast * contrast_loss
                        +config.LOSS.cont * cont_loss
                        +config.LOSS.iou * iou_loss
                        +config.LOSS.bce * bce_loss
                    )
                    # total_loss = config.LOSS.LOCALIZATION * loc_loss + config.LOSS.MATCH * match_loss

                if global_step % 50 == 0:
                    logger.info(
                        "epoch: {}, step: {}/{}, lr: {:.6f},  total: {:.4f}, loc: {:.4f}, match: {:.4f}, kl: {:.6f}, early: {:.4f},bce: {:.4f},iou: {:.4f}".format(
                            epoch + 1,
                            step,
                            step_epoch,
                ...
                writer.add_scalar("Loss_train/iou", iou_loss, global_step)
                writer.add_scalar("Loss_train/bce", bce_loss, global_step)
                writer.add_scalar(
                    "learning_rate",
                    optimizer.state_dict()["param_groups"][0]["lr"],
                    global_step,
            ...
            if config.TEST.EVAL_TRAIN:
                # trainset eval
                if torch.cuda.device_count() > 1: 
                    r1i3, r1i5, r1i7, mi, score_str, statistics_str = model.module.eval_test(
                        model=model.module,
        ...
        with torch.no_grad():
            for idx, batch_data in enumerate(dataloader):
                ...
                if torch.cuda.device_count() > 1:
                    start_indices, end_indices = model.module.extract_index(
